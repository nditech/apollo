...
from apollo.models import Form, Message, Participant, Submission, SubmissionVersion
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("extra_data", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column(
            "submission_type", sqlalchemy_utils.types.choice.ChoiceType(Submission.SUBMISSION_TYPES), nullable=True
        ),
        sa.Column("created", sa.DateTime(), nullable=True),
        sa.Column("updated", sa.DateTime(), nullable=True),
        sa.Column("sender_verified", sa.Boolean(), nullable=True),
        sa.Column(
            "quarantine_status", sqlalchemy_utils.types.choice.ChoiceType(Submission.QUARANTINE_STATUSES), nullable=True
        ),
        sa.Column(
            "verification_status",
            sqlalchemy_utils.types.choice.ChoiceType(Submission.VERIFICATION_STATUSES),
            nullable=True,
        ),
        sa.Column("incident_description", sa.String(), nullable=True),
        sa.Column(
            "incident_status", sqlalchemy_utils.types.choice.ChoiceType(Submission.INCIDENT_STATUSES), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "channel", sqlalchemy_utils.types.choice.ChoiceType(SubmissionVersion.CHANNEL_CHOICES), nullable=True
        ),
        sa.Column("deployment_id", sa.Integer(), nullable=False),
        ...
        if isinstance(item, Permission):
            for need in item.needs:
                if need.method == "action":
                    conn.execute(
                        text("""INSERT INTO permission (name, deployment_id, uuid)
                                 VALUES (:value, 1, :uuid)"""),
                        {"uuid": uuid4().hex, "value": need.value},
                    )

    # clerk
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
        ...
        {"role": "clerk", "permission": "add_submission"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "clerk", "permission": "edit_both_submissions"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "clerk", "permission": "edit_submission"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "clerk", "permission": "view_messages"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "clerk", "permission": "view_participants"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "clerk", "permission": "view_events"},
    )
    ...
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 ...
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "add_submission"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "edit_both_submissions"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "edit_submission"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "edit_submission_quarantine_status"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "edit_submission_verification_status"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "send_messages"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "view_events"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "view_messages"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "view_participants"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "manager", "permission": "view_quality_assurance"},
    )
    ...
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 ...
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "add_submission"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "edit_both_submissions"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "edit_participant"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "edit_submission"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "edit_submission_quarantine_status"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "edit_submission_verification_status"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "export_messages"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "export_participants"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "export_submissions"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "send_messages"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "view_events"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "view_messages"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "view_participants"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "view_quality_assurance"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "view_process_analysis"},
    )
    conn.execute(
        text("""INSERT INTO roles_permissions (role_id, permission_id)
                 VALUES ((SELECT id FROM role WHERE deployment_id=1 AND name=:role),
                 (SELECT id FROM permission WHERE deployment_id=1 AND name=:permission))
                 ON CONFLICT DO NOTHING"""),
        {"role": "analyst", "permission": "view_result_analysis"},
    )
