...
def upgrade():
    ...
    op.add_column("form", sa.Column("vote_shares", JSONB(astext_type=sa.Text()), nullable=True))
    ...
    for row in connection.execute(statement):
        ...
        if form_data:
            field_cache = {f["tag"]: f for g in form_data.get("groups", []) for f in g.get("fields", [])}
        else:
            field_cache = {}
        ...
        op.execute(
            form_table.update()
            .where(form_table.c.id == row[form_table.c.id])
            .values({"data": form_data, "vote_shares": vote_shares})
        )
...
def downgrade():
    ...
    for row in connection.execute(statement):
        ...
        op.execute(form_table.update().where(form_table.c.id == row[form_table.c.id]).values({"data": form_data}))
