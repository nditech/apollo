...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Form.activity'
        db.delete_column('core_form', 'activity_id')

        # Adding M2M table for field forms on 'Activity'
        db.create_table('core_activity_forms', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('activity', models.ForeignKey(orm['core.activity'], null=False)),
            ('form', models.ForeignKey(orm['core.form'], null=False))
        ))
        db.create_unique('core_activity_forms', ['activity_id', 'form_id'])


    def backwards(self, orm):
        # Adding field 'Form.activity'
        db.add_column('core_form', 'activity',
                      self.gf('django.db.models.fields.related.ForeignKey')(related_name='forms', null=True, to=orm['core.Activity'], blank=True),
                      keep_default=False)

        # Removing M2M table for field forms on 'Activity'
        db.delete_table('core_activity_forms')


    models = {
        'core.activity': {
            'Meta': {'object_name': 'Activity'},
            'date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2013, 3, 7, 0, 0)'}),
