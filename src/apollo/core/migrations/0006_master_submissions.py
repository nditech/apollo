...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for submission in orm.Submission.objects.filter(form__type='CHECKLIST').exclude(observer=None):
            try:
                master = orm.Submission.objects.get(form=submission.form,\
                        date=submission.date, location=submission.location, observer=None)
                submission.master = master
                submission.save()
            except orm.Submission.DoesNotExist:
                pass
    ...
    models = {
        'core.activity': {
            ...
            'date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2013, 2, 13, 0, 0)'}),
