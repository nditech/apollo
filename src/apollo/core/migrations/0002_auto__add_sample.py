...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'Sample'
        db.create_table('core_sample', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('name', self.gf('django.db.models.fields.CharField')(max_length=100)),
        ))
        db.send_create_signal('core', ['Sample'])

        # Adding M2M table for field locations on 'Sample'
        db.create_table('core_sample_locations', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('sample', models.ForeignKey(orm['core.sample'], null=False)),
            ('location', models.ForeignKey(orm['core.location'], null=False))
        ))
        db.create_unique('core_sample_locations', ['sample_id', 'location_id'])


    def backwards(self, orm):
        # Deleting model 'Sample'
        db.delete_table('core_sample')

        # Removing M2M table for field locations on 'Sample'
        db.delete_table('core_sample_locations')
