...
def provision(environment="app", server="staging"):
    ...
    if confirm("Provision %s server?" % (environment,)):
        if environment == "app":
            ...
            sudo('apt-get install -y vim nginx memcached python-dev build-essential git-core libpq-dev postgresql-client rabbitmq-server librabbitmq-dev libxml2-dev libxslt-dev libproj-dev binutils gdal-bin gettext')
    ...
    if confirm("Deploy %s to %s server?" % (version, server)):
        ...
        if server in ["production", "staging"]:
            ...
            with cd('%s/%s' % (root_dir, SCRIPT_NAME)):
                ...
                sudo('ln -sf `pwd`/parts/nginx/%s.conf /etc/nginx/sites-enabled/' % (SCRIPT_NAME,))
