...
def eday_L(q=Q()):
    l = len(set(EDAYChecklist.objects.filter(q).values_list('observer__lga__code', flat=True)))
    return l
    
def eday_QAA(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(AA__gt=0).values_list('AA', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(AA__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('AA')[0].help_text }

def eday_QBF(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BF__gt=0).values_list('BF', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BF__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BF')[0].help_text }

def eday_QBK(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BK__gt=0).values_list('BK', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BK__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BK')[0].help_text }

def eday_QBN(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BN__gt=0).values_list('BN', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BN__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BN')[0].help_text }

def eday_QCB(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CB__gt=0).values_list('CB', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CB__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CB')[0].help_text }

def eday_QCF(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CF__gt=0).values_list('CF', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CF__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CF')[0].help_text }

def eday_QCG(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CG__gt=0).values_list('CG', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CG__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CG')[0].help_text }

def eday_QCH(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CH__gt=0).values_list('CH', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CH__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CH')[0].help_text }

def eday_QCJ(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CJ__gt=0).values_list('CJ', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CJ__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CJ')[0].help_text }

def eday_QCK(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CK__gt=0).values_list('CK', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CK__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CK')[0].help_text }

def eday_QCM(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CM__gt=0).values_list('CM', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CM__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CM')[0].help_text }

def eday_QCN(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CN__gt=0).values_list('CN', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CN__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CN')[0].help_text }

def eday_QCP(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CP__gt=0).values_list('CP', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CP__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CP')[0].help_text }

def eday_QCQ(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CQ__gt=0).values_list('CQ', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CQ__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CQ')[0].help_text }

def eday_QBC(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BC__gt=0).values_list('BC', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BC__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BC')[0].help_text }

def eday_QBA(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BA__gt=0).values_list('BA', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BA__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BA')[0].help_text }

def eday_QBG(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BG__gt=0).values_list('BG', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BG__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BG')[0].help_text }

def eday_QBH(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BH__gt=0).values_list('BH', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BH__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BH')[0].help_text }

def eday_QBJ(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BJ__gt=0).values_list('BJ', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BJ__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BJ')[0].help_text }

def eday_QBM(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(BM__gt=0).values_list('BM', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BM__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('BM')[0].help_text }

def eday_QCA(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CA__gt=0).values_list('CA', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CA__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CA')[0].help_text }

def eday_QCC(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CC__gt=0).values_list('CC', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CC__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CC')[0].help_text }

def eday_QCD(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CD__gt=0).values_list('CD', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CD__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CD')[0].help_text }

def eday_QCE(q=Q()):   
    edays = EDAYChecklist.objects.filter(q).filter(CE__gt=0).values_list('CE', flat=True)
    n = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(CE__gt=0).values_list('observer__lga__code', flat=True)))
    ...
    for eday in edays:
        try:
            ...
            pass
    return {'l': l, 'n': n, 'options': options, 'label': EDAYChecklist._meta.get_field_by_name('CE')[0].help_text }

def eday_QBD(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(BD__isnull=False).values_list('BD', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BD__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('BD')[0].help_text }

def eday_QBE(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(BE__isnull=False).values_list('BE', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BE__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('BE')[0].help_text }

def eday_QBP(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(BP__isnull=False).values_list('BP', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(BP__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('BP')[0].help_text }

def eday_QDA(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(DA__isnull=False).values_list('DA', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(DA__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('DA')[0].help_text }

def eday_QDB(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(DB__isnull=False).values_list('DB', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(DB__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('DB')[0].help_text }

def eday_QDC(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(DC__isnull=False).values_list('DC', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(DC__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('DC')[0].help_text }

def eday_QDD(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(DD__isnull=False).values_list('DD', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(DD__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('DD')[0].help_text }

def eday_QDE(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(DE__isnull=False).values_list('DE', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(DE__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('DE')[0].help_text }

def eday_QDF(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(DF__isnull=False).values_list('DF', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(DF__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('DF')[0].help_text }

def eday_QDG(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(DG__isnull=False).values_list('DG', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(DG__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('DG')[0].help_text }

def eday_QDH(q=Q()):
    edays = EDAYChecklist.objects.filter(q).filter(DH__isnull=False).values_list('DH', flat=True)
    total = len(edays)
    l = len(set(EDAYChecklist.objects.filter(q).filter(DH__isnull=False).values_list('observer__lga__code', flat=True)))
    ...
    return {'l': l, 'n': total, 'valid_n': len(valid), 'mean': mean(valid), 'label': EDAYChecklist._meta.get_field_by_name('DH')[0].help_text }
