var AdminFilters = function (element, filtersElement, filterGroups, activeFilters) {
    ...
    function addFilter(name, subfilters, selectedIndex, filterValue) {
        $('#filter_form').removeClass('d-none');
        ...
        $el.append(
            $('<td/>').append(
                $('<a href="#" class="btn btn-block text-left btn-secondary remove-filter" />')
        ...
        var $select = $('<select class="form-control filter-op" />');
        ...
        $select.select2({ width: 'resolve', theme: 'bootstrap4' }).on("change", function (e) {
