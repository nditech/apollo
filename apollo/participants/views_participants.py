...
bp = Blueprint(
    "participants", __name__, template_folder="templates", static_folder="static", static_url_path="/core/static"
)
...
bp.add_url_rule(
    ...
    view_func=api_views.ParticipantItemResource.as_view("api_participant_item"),
)
bp.add_url_rule("/api/participants/", view_func=api_views.ParticipantListResource.as_view("api_participant_list"))
bp.add_url_rule(
...
docs.register(api_views.ParticipantItemResource, "participants.api_participant_item")
docs.register(api_views.ParticipantListResource, "participants.api_participant_list")

admin_required = permissions.role("admin").require


def has_participant_set():
    """Returns a boolean signifying if the current event has a participant set."""
    return g.event.participant_set is not None


@route(bp, "/participants", endpoint="participant_list", methods=["GET", "POST"])
@register_menu(
    bp,
    "main.participants",
    _("Participants"),
    icon='<i class="glyphicon glyphicon-user"></i>',
    visible_when=lambda: permissions.view_participants.can() and has_participant_set(),
    order=5,
)
@login_required
...
def participant_list(participant_set_id=0, view=None):
    ...
    if participant_set_id:
        participant_set = ParticipantSet.query.filter(ParticipantSet.id == participant_set_id).first_or_404()
        template_name = "admin/participant_list.html"
        breadcrumbs = [
            {"url": url_for("participantset.index_view"), "text": _("Participant Sets")},
    ...
    extra_fields = (
        [field for field in participant_set.extra_fields if field.visible_in_lists]
    ...
    if request.args.get("location"):
        location = Location.query.filter(Location.id == request.args.get("location")).first()
    ...
    queryset = (
        Participant.query.options(joinedload(Participant.samples))
        .select_from(
            Participant,
        )
        .filter(Participant.participant_set == participant_set)
    )

    # load the location set linked to the participant set
    location_set_id = participant_set.location_set_id
    filter_class = filters.participant_filterset(participant_set.id, location_set_id)
    ...
    if request.args.get("export") and permissions.export_participants.can():
        ...
        basename = slugify(
            "%s participants %s" % (participant_set.name.lower(), datetime.utcnow().strftime("%Y %m %d %H%M%S"))
        )
    ...
    if request.args.get("sort_by") == "location" and request.args.get("sort_value"):
        ...
        queryset = (
            models.Participant.query.select_from(
                models.Participant,
                models.Location,
            )
            .filter(models.Participant.participant_set_id == participant_set.id)
            .outerjoin(models.Location, models.Participant.location_id == models.Location.id)
            .outerjoin(descendants, descendants.c.descendant_id == models.Participant.location_id)
            .group_by(descendants.c.name_translations, models.Participant.id)
        )
    elif request.args.get("sort_by") == "phone":
        queryset = (
            models.Participant.query.filter(models.Participant.participant_set_id == participant_set.id)
            .outerjoin(models.Location, models.Participant.location_id == models.Location.id)
        ...
        queryset = (
            ...
            .filter(models.Participant.participant_set_id == participant_set.id)
            .outerjoin(models.Location, models.Participant.location_id == models.Location.id)
            .outerjoin(models.ParticipantRole, models.Participant.role_id == models.ParticipantRole.id)
            .outerjoin(models.ParticipantPartner, models.Participant.partner_id == models.ParticipantPartner.id)
        )

    if request.args.get("sort_by") == "id":
        if request.args.get("sort_direction") == "desc":
            queryset = queryset.order_by(desc(models.Participant.participant_id.cast(BigInteger)))
        else:
            queryset = queryset.order_by(models.Participant.participant_id.cast(BigInteger))
        ...
        queryset = queryset.order_by(
            models.Location.code.cast(BigInteger), models.Participant.participant_id.cast(BigInteger)
        )
    ...
    if request.form.get("action") == "send_message":
        ...
        recipients = [
            x
            for x in [
                participant.primary_phone if participant.primary_phone else "" for participant in queryset_filterset.qs
            ]
            if x != ""
        ]
    ...
    context = {
        ...
        "location_types": helpers.displayable_location_types(is_administrative=True, location_set_id=location_set_id),
        "participants": queryset_filterset.qs.paginate(page=page, per_page=current_app.config.get("PAGE_SIZE")),
    }
...
def toggle_phone_verification():
    ...
    if request.accept_mimetypes.accept_json:
        ...
        phone_contact = PhoneContact.query.filter(
            PhoneContact.participant_id == participant_id, PhoneContact.number == phone
        ).one()
        phone_contact.verified = not phone_contact.verified
        phone_contact.save()
        Submission.query.filter(
            Submission.participant == phone_contact.participant, Submission.last_phone_number == phone
        ).update({Submission.sender_verified: phone_contact.verified})
        db.session.commit()
...
@route(bp, "/participant/<int:id>", endpoint="participant_edit", methods=["GET", "POST"])
@login_required
@permissions.edit_participant.require(403)
def participant_edit(id, participant_set_id=0, view=None):
    """Participant edit view."""
    participant = Participant.query.get_or_404(id)
    breadcrumbs = [_("Participants"), _("Edit Participant"), participant.participant_id]
    ...
    if request.method == "GET":
        ...
        if form.validate():
            ...
            for locale in deployment.locale_codes:
                ...
                first_name_translations[locale] = getattr(form, field_name).data
            ...
            if form.role.data:
                participant.role_id = ParticipantRole.query.get_or_404(form.role.data).id
            ...
            if form.partner.data:
                participant.partner_id = ParticipantPartner.query.get_or_404(form.partner.data).id
            else:
            ...
            phone = PhoneContact.query.filter_by(number=phone_number, participant_id=id).first()

            if phone is None:
                # no, overwrite the primary phone,
                # or create a new primary phone
                primary_num = participant.primary_phone
                phone = PhoneContact.query.filter_by(number=primary_num, participant_id=id, verified=True).first()
                if phone:
                    phone.number = phone_number
                else:
                    phone = PhoneContact(number=phone_number, participant_id=id, verified=True)
            else:
            ...
            if participant_set_id:
                return redirect(
                    url_for("participants.participant_list_with_set", participant_set_id=participant_set.id)
                )
            else:
...
@route(bp, "/participants/import", endpoint="participant_list_import", methods=["POST"])
@login_required
@permissions.import_participants.require(403)
def participant_list_import(participant_set_id=0):
    """Participants import view."""
    if participant_set_id:
    ...
    if not form.validate():
        ...
        upload = services.user_uploads.create(deployment_id=g.deployment.id, upload_filename=filename, user_id=user.id)

    if participant_set_id:
        return redirect(
            url_for("participantset.participants_headers", participant_set_id=participant_set.id, upload_id=upload.id)
        )
    else:
        return redirect(url_for("participants.participant_headers", upload_id=upload.id))


@route(bp, "/participants/headers/<int:upload_id>", endpoint="participant_headers", methods=["GET", "POST"])
@login_required
@permissions.import_participants.require(403)
def participant_headers(upload_id, participant_set_id=0, view=None):
    """View for returning the headers for the import."""
    if participant_set_id:
        participant_set = ParticipantSet.query.get_or_404(participant_set_id)
    else:
        participant_set = g.event.participant_set or abort(404)
    ...
    upload = UserUpload.query.filter(UserUpload.id == upload_id, UserUpload.user == user).first_or_404()
    ...
    if request.method == "GET":
        ...
        if not form.validate():
            ...
            if view:
                return view.render("frontend/participant_headers_errors.html", error_msgs=error_msgs), 400
            else:
                return render_template("frontend/participant_headers_errors.html", error_msgs=error_msgs), 400
        else:
            ...
            if participant_set_id:
                return redirect(url_for("participantset.participants_list", participant_set_id=participant_set_id))
            else:
                return redirect(url_for("participants.participant_list"))
...
def log_call():
    ...
    if request.accept_mimetypes.accept_json:
        ...
        user = current_user._get_current_object()
        contact = models.ContactHistory(participant=participant, user=user, description=description)
