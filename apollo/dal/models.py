...
def get_default_locale(obj, attr):
    ...
    try:
        ...
        warnings.warn("No Deployment Set", stacklevel=2)
        locale = "en"
    except RuntimeError:
        raise
...
def expr_factory(self, attr):
    """Override the default `expr_factory` in TranslationHybrid.
    ...
    def expr(cls):
        ...
        return sa.func.coalesce(
            cls_attr.op("->>")(current_locale),
            sa.func.coalesce(
                cls_attr.op("->>")(default_locale), sa.func.jsonb_path_query_first(cls_attr, "$.*").cast(sa.String)
            ),
        )
...
translation_hybrid = TranslationHybrid(current_locale=get_locale, default_locale=get_default_locale)
...
class Permission(BaseModel):
    ...
    PERMISSION_DESCRIPTIONS = {
        ...
        "view_quality_assurance": _("Users can view the quality assurance list"),
        ...
        "edit_both_submissions": _("Users can modify observer and location checklists"),
        "edit_submission_quarantine_status": _("Users can edit checklist quarantine statuses"),
        "edit_submission_verification_status": _("Users can edit checklist verification statuses"),
    ...
    deployment = db.relationship("Deployment", backref=db.backref("permissions", cascade="all, delete"))
...
class ResourceMixin(object):
    """Resource mixin class.

    Any resources to be protected should inherit from the concrete class, `Resource`, not this one.
    ...
    def __tablename__(self):
        """What database table to use for this resource."""
        return "resource"

    @declared_attr
    def resource_id(self):
        return db.Column(db.Integer, autoincrement=True, nullable=False, primary_key=True)
    ...
    def deployment_id(self):
        return db.Column(db.Integer, db.ForeignKey("deployment.id", ondelete="CASCADE"), nullable=False)

    @declared_attr
    def deployment(self):
        return db.relationship(
            "Deployment", backref=db.backref("resources", cascade="all, delete", passive_deletes=True)
        )

    @declared_attr
    def roles(self):
        return db.relationship("Role", backref="resources", secondary="role_resource_permissions")

    @declared_attr
    def users(self):
        return db.relationship("User", backref="resources", secondary="user_resource_permissions")
...
class Resource(ResourceMixin, BaseModel):
    ...
    Serves as a registry and base class for resources, that is items that may be protected.
