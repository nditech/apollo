...
from apollo import helpers, models
...
def init_submissions(self, event_id, form_id, role_id, location_type_id, channel=None):
    """Initialize checklist submissions in preparation to receive data."""
    ...
    location_type = models.LocationType.query.filter_by(id=location_type_id).first()
...
def init_survey_submissions(self, event_id, form_id, upload_id):
    """Initialize survey submissions in preparation to receive data."""
    event = models.Event.query.filter_by(id=event_id).first()
    form = models.Form.query.filter_by(id=form_id).first()
    ...
    with open(filepath, "rb") as f:
        ...
        for idx in range(len(dataframe.index)):
            ...
            participant = Participant.query.filter(
                Participant.participant_id == pid, Participant.participant_set == event.participant_set
            ).first()

            if not participant:
                error_records += 1
                error_log.append(
                    {"label": "ERROR", "message": gettext("Participant ID %(part_id)s was not found", part_id=pid)}
                )
            ...
            if not serial:
                ...
                error_log.append(
                    {
                        "label": "ERROR",
                        "message": gettext("No form serial number specified on line %(lineno)d", lineno=idx + 1),
                    }
                )
