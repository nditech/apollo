...
class SubmissionSerializer(object):
    ...
    def serialize_one(self, obj):
        ...
        return {
            ...
            'participant': obj.participant.uuid.hex if obj.participant else None,
            ...
            'updated': calendar.timegm(obj.updated.timetuple()) if obj.updated else None,
            'sender_verified': obj.sender_verified,
            'quarantine_status': obj.quarantine_status.code if obj.quarantine_status else '',
            'verification_status': obj.verification_status.code if obj.verification_status else '',
            'incident_description': obj.incident_description,
            'incident_status': obj.incident_status.code if obj.incident_status else None,
...
class SubmissionArchiveSerializer(ArchiveSerializer):
    ...
    def serialize(self, query, zip_file):
        query = query.join(Form).join(Location).join(Participant)
