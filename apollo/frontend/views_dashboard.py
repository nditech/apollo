...
from ..frontend.dashboard import event_days, get_coverage, get_daily_progress, get_stratified_daily_progress
from ..frontend.helpers import get_checklist_form_dashboard_menu, get_concurrent_events_list_menu, get_event, set_event
...
def main_dashboard(form_id=None):
    ...
    if data_view:
        session["dashboard_data_view"] = data_view if data_view in ["divisions", "locations"] else "divisions"
    daily_progress = {}
    ...
    if not form_id:
        ...
        form = Form.query.filter(
            Form.events.contains(event), Form.form_type.in_(["CHECKLIST", "SURVEY"]), Form.id == form_id
        ).first_or_404()
    ...
    query_args = [Submission.event_id == event.id, Submission.form_id == form.id] if form else [false()]
    if form:
    ...
    query = Submission.query.filter(*query_args).join(Location, Submission.location_id == Location.id)
        ...
        if participant:
            if group_slug:
                _location_query = (
                    ...
                    .filter(models.LocationPath.ancestor_id.in_([loc.id for loc in participant.location.parents()]))
                )
            else:
                _location_query = (
                    models.Location.query.with_entities(models.Location.id)
    ...
    if args.get("location_"):
        location = Location.query.filter(
            Location.id == args.get("location_"), Location.location_set_id == event.location_set_id
        ).first_or_404()

    if group_slug and location and session.get("dashboard_data_view") == "locations":
        _location_query = (
            models.Location.query.with_entities(models.Location.id)
            .join(models.LocationPath, models.Location.id == models.LocationPath.descendant_id)
    ...
    if not group_slug:
        ...
        if form and form.show_progress and not stratified_progress:
            ...
            admin_location_types = (
                LocationType.query.filter(
                    LocationType.is_administrative == True,  # noqa
                    LocationType.location_set_id == event.location_set_id,
                )
                .join(LocationType.ancestor_paths)
                .group_by(LocationType.id, models.LocationTypePath.depth)
                .order_by(models.LocationTypePath.depth)
                .all()
            )
            ...
            if location_type_id:
                ...
                index, location_type = next(
                    (pair for pair in enumerate(admin_location_types) if str(pair[1].id) == location_type_id),
                    (None, None),
                )
            ...
            daily_stratified_progress = get_stratified_daily_progress(query, event, location_type)
    else:
        group = next((grp for grp in form.data["groups"] if grp["slug"] == group_slug), None)
        if group is None:
            abort(404)

        if location:
            breadcrumbs.append(
                {"text": group["name"], "url": url_for("dashboard.response_rate", form_id=form.id, group=group_slug)}
            )
        else:
        ...
        if location:
            for ancestor in location.ancestors():
                if ancestor.location_type in admin_location_types:
                    idx, lt = next(
                        (pair for pair in enumerate(admin_location_types) if pair[1].id == ancestor.location_type.id),
                        (None, None),
                    )
                    try:
                        breadcrumbs.append(
        ...
        if location_type_id:
            ...
            # is a string
            index, location_type = next(
                (pair for pair in enumerate(admin_location_types) if str(pair[1].id) == location_type_id), (None, None)
            )
        ...
        data = get_coverage(query_filterset.qs, form, group, location_type)
...
@route(bp, "/")
@register_menu(bp, "main.dashboard", _("Dashboard"), order=0)
@login_required
def index():
    """Dashboard view."""
...
@register_menu(
    bp,
    "main.dashboard.response_rate",
    _("Response Rate"),
    order=0,
    visible_when=lambda: len(
        get_checklist_form_dashboard_menu(form_type="CHECKLIST") + get_checklist_form_dashboard_menu(form_type="SURVEY")
    )
    > 0,
    dynamic_list_constructor=lambda: get_checklist_form_dashboard_menu(form_type="CHECKLIST")
    + get_checklist_form_dashboard_menu(form_type="SURVEY"),
)
@login_required
def response_rate(form_id=None):
    """Response rate dashboard view."""
...
def concurrent_events(event_id):
    """View for switching between concurrent events."""
...
@register_menu(bp, "user.events", _("Events"), visible_when=partial(lambda: permissions.view_events.can()))
@login_required
