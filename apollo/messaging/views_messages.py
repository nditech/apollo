...
def message_list():
    """View for rendering the messages list."""
    ...
    qs = Message.query.filter(Message.deployment == deployment, Message.event_id.in_(event_ids)).order_by(
    ...
    if request.args.get("export") and permissions.export_messages.can():
        ...
        basename = slugify("%s messages %s" % (g.event.name.lower(), datetime.utcnow().strftime("%Y %m %d %H%M%S")))
        ...
        split_messages = (
            ...
            .outerjoin(Submission, Message.submission_id == Submission.id)
        ...
        # filtering
        all_messages = split_messages.with_entities(Message, OutboundMsg, Submission.id, Form.form_type).order_by(
            Message.received.desc()
        )

        if "mobile" not in filter_errors and filter_data.get("mobile"):
            search_term = filter_data.get("mobile")
            numbers = list(chain(*[term.strip().split() for term in search_term.split(",") if not term.isspace()]))
        ...
        if "date" not in filter_errors and filter_data.get("date"):
            try:
                ...
                all_messages = all_messages.filter(
                    sa.or_(
                        sa.and_(Message.received >= date_start, Message.received <= date_end),
                        sa.and_(OutboundMsg.received >= date_start, OutboundMsg.received <= date_end),
                    )
                )
        ...
        if "form_type" not in filter_errors and (filter_data.get("form_type") != ""):
            if filter_data.get("form_type") == "Invalid":
                ...
                all_messages = all_messages.filter(Form.form_type == filter_data.get("form_type"))
        ...
        context = {
            ...
            "args": data,
            "pager": all_messages.paginate(page=page, per_page=current_app.config.get("PAGE_SIZE")),
...
def message_time_series(message_queryset):
    """Generates the time series for the messagges queryset."""
    ...
    if df.empty is not True:
        ...
        data = {
            "incoming": [
                (calendar.timegm(i[0].utctimetuple()) * 1000, int(i[1])) for i in sorted(df.to_dict()["marker"].items())
            ]
        }
    else:
        data = {"incoming": []}
