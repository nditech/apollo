...
def make_message_text(form, participant, data, serial: str = None) -> str:
    """Generate message text from form data."""
...
def generate_config_qr_code(participant=None):
    """Generate configuration QR code."""
    settings = {"general": {"server_url": urljoin(url_for("dashboard.index", _external=True), "xforms")}, "admin": {}}

    if participant is not None:
        settings["general"].update(username=participant.participant_id, password=participant.password)

    json_bytes = json.dumps(settings).encode("UTF-8")
    compressed_json = zlib.compress(json_bytes)

    qr_code = qrcode.QRCode(version=1, error_correction=qrcode.ERROR_CORRECT_L, box_size=10, border=4)
