...
from flask import Blueprint, abort, flash, g, jsonify, redirect, request, send_file, session, url_for
...
from apollo.formsframework.forms import FormForm, FormImportForm, make_questionnaire_hidden_toggle_form
from apollo.formsframework.models import FormBuilderSerializer
from apollo.frontend.forms import make_checklist_init_form, make_survey_init_form
...
bp = Blueprint("forms", __name__, template_folder="templates", static_folder="static")

bp.add_url_rule("/api/forms", view_func=api_views.FormListResource.as_view("api_form_list"))
bp.add_url_rule("/api/forms/<int:form_id>", view_func=api_views.FormItemResource.as_view("api_form_item"))
...
def checklist_init():
    ...
    if form.validate_on_submit():
        questionnaire = models.Form.query.filter_by(id=form.data.get("form")).one()

        if len(questionnaire.tags) == 0:
            flash_category = "danger"
            flash_message = _("Checklists were not created, as the selected Form " "has no question codes")
        else:
            flash_category = "info"
            flash_message = _(
                "Checklists are being created for the Event, Form, " "Role and Location Type you selected"
            )
...
def survey_init():
    ...
    if form.validate_on_submit():
        ...
        if len(questionnaire.tags) == 0:
            flash_category = "danger"
            flash_message = _("Surveys were not created as the selected Form " "has no question codes")
        else:
            flash_category = "info"
            flash_message = _("Surveys are being created for the Event, Form " "and Participants you selected")

            user = current_user._get_current_object()
            upload_file = strip_bom_header(request.files["import_file"])
            filename = uploads.save(upload_file)
            upload = UserUpload(deployment_id=g.deployment.id, upload_filename=filename, user_id=user.id)
            upload.save()

            task_kwargs = {"event_id": form.data["event"], "form_id": form.data["form"], "upload_id": upload.id}
...
def form_builder(view, id):
    ...
    breadcrumbs = [{"text": _("Forms"), "url": url_for("formsview.index")}, _("Form Builder"), form.name]

    ctx = {"breadcrumbs": breadcrumbs, "form": form}
...
def new_form(view):
    ...
    breadcrumbs = [{"text": _("Forms"), "url": url_for("formsview.index")}, _("Create Form")]
    ...
    roles = models.Role.query.filter_by(deployment_id=g.event.deployment_id).all()
    form.roles = roles
    form.save()
...
def edit_form(view, form_id):
    ...
    breadcrumbs = [{"text": _("Forms"), "url": url_for("formsview.index")}, _("Edit Form")]
    field_options = [("", "")] + [(tag, tag) for tag in form.tags]
    ...
    web_form.vote_shares.choices = [] + [(tag, tag) for tag in form.tags]
    web_form.turnout_fields.choices = [] + [(tag, tag) for tag in form.tags]
    web_form.result_images.choices = [] + [
        (tag, tag) for tag in form.tags if form.get_field_by_tag(tag).get("type") == "image"
    ]
...
def forms_list(view):
    ...
    if show_hidden:
        ...
        query = query.filter(models.Form.is_hidden == False).order_by("name")  # noqa
    ...
    if show_hide_form.validate_on_submit():
        ...
        models.Form.query.filter(models.Form.id.in_(posted_form_ids)).update({"is_hidden": hide_forms})
        db.session.commit()
    ...
    context = {
        ...
        "events": models.Event.query.filter(models.Event.is_hidden == False).order_by("name").all(),  # noqa
...
def quality_controls(view, form_id):
    ...
    form = models.Form.query.filter_by(id=form_id).first_or_404()
    breadcrumbs = [{"text": _("Forms"), "url": url_for("formsview.index")}, _("Quality Assurance"), form.name]
    ...
    context = {"breadcrumbs": breadcrumbs, "form": form, "quality_controls": quality_controls}

    return view.render(template_name, **context)


def sort_quality_controls(view, form_id):
    """Sorted view of quality assurance controls."""
    form = models.Form.query.filter_by(id=form_id).first_or_404()
...
def quality_control_edit(view, form_id, qc=None):
    """Quality assurance control editing view."""
    template_name = "admin/quality_assurance_edit.html"
    form = models.Form.query.filter_by(id=form_id).first_or_404()
    breadcrumbs = [
        {"text": _("Forms"), "url": url_for("formsview.index")},
        {"text": _("Quality Assurance"), "url": url_for("formsview.qc", form_id=form.id)},
        form.name,
    ]
    ...
    if request.method == "POST":
        try:
            ...
            if "name" in postdata and postdata["name"]:
                ...
                quality_control = next(filter(lambda c: c["name"] == postdata["name"], form.quality_checks))
    ...
    if qc:
        try:
            quality_check = next(filter(lambda c: c["name"] == qc, form.quality_checks))
        ...
        quality_control = {
            "name": "",
            "description": "",
            "criteria": [{"lvalue": "", "comparator": "=", "rvalue": "", "conjunction": "&&", "id": "0"}],
        }
...
def export_form(id):
    ...
    filename = slugify(f"{form.name}-{now:%Y %m %d %H%M%S}") + ".xls"

    return send_file(memory_file, download_name=filename, as_attachment=True, mimetype="application/vnd.ms-excel")


def import_form_schema():
    """Import form schema view."""
    web_form = FormImportForm()

    if web_form.validate_on_submit():
        form = utils.import_form(request.files["import_file"].stream)
