...
FIELD_TYPES = ("comment", "integer", "select", "multiselect", "string", "image")
...
events_forms = db.Table(
    ...
    db.Column("form_id", db.Integer, db.ForeignKey("form.id", ondelete="CASCADE"), primary_key=True),
)


class Form(Resource):
    FORM_TYPES = (("CHECKLIST", _("Checklist Form")), ("INCIDENT", _("Incident Form")), ("SURVEY", _("Survey Form")))
    ...
    is_hidden = db.Column(db.Boolean, default=False, nullable=False, server_default=false())
    ...
    version_identifier = db.Column(db.String, default=_make_version_identifer, onupdate=_make_version_identifer)
    resource_id = db.Column(db.Integer, db.ForeignKey("resource.resource_id", ondelete="CASCADE"))
    ...
    def _populate_field_cache(self):
        if self.data:
            self._field_cache = {f["tag"]: f for g in self.data.get("groups", []) for f in g.get("fields", [])}
        else:
            self._field_cache = {}

    def _populate_group_cache(self):
        if self.data:
            self._group_cache = {g["name"]: g for g in self.data.get("groups", [])}
        else:
            self._group_cache = {}
    ...
    def qa_tags(self):
        ...
        return sorted(tag for tag, field in self._field_cache.items() if field["type"] in QA_FIELD_TYPES)
    ...
    def vote_tags(self):
        ...
        return sorted([key for key in self._field_cache.keys() if self._field_cache[key]["analysis_type"] == "RESULT"])
    ...
    def get_image_fields(self):
        return [tag for tag in self.tags if self.get_field_by_tag(tag)["type"] == "image"]
    ...
    def to_xml(self):
        ...
        data.attrib["{{{}}}version".format(NSMAP["orx"])] = self.version_identifier
            ...
            for group in self.data.get("groups"):
                ...
                for field in group["fields"]:
                    ...
                    if field_type == "boolean":
                        ...
                        field_element = E.input(E.label(field["description"]), ref=field["tag"])
                        ...
                        model.append(
                            E.bind(
                                nodeset=path,
                                type="integer",
                                constraint=". >= {} and . <= {}".format(field.get("min", 0), field.get("max", 9999)),
                            )
                        )
                        ...
                        field_element = element_factory(E.label(field["description"]), ref=field["tag"])
                        for key, value in sorted_options:
                            field_element.append(E.item(E.label(key), E.value(str(value))))
                    elif field_type == "image":
                        mediatype = "image/*"
                        field_element = E.upload(E.label(field["description"]), ref=field["tag"], mediatype=mediatype)
    ...
    def odk_hash(self):
        xform_data = etree.tostring(self.to_xml(), encoding="UTF-8", xml_declaration=True)
        hash_engine = hashlib.md5()
        hash_engine.update(xform_data)
        return f"md5: {hash_engine.hexdigest()}"

    def create_schema(self):
        attrs = {tag: _get_schema_field(self.get_field_by_tag(tag)) for tag in self.tags}

        attrs["location"] = fields.List(fields.Float(), validate=validate.Length(min=2, max=2))

        attrs["Meta"] = type("GeneratedMeta", (getattr(Schema, "Meta", object),), {"register": False})
...
def _get_schema_field(form_field):
    if form_field["type"] == "integer":
        return fields.Int(validate=validate.Range(min=form_field.get("min", 0), max=form_field.get("max", 9999)))
...
class FormBuilderSerializer(object):
    ...
    def serialize_group(cls, group):
        ...
        field_data.append({"label": group["name"], "component": "group"})
    ...
    def deserialize(cls, form, data):
        ...
        if len(data["fields"]) > 0:
            if data["fields"][0]["component"] != "group":
                ...
                group = {"name": _("SMS 1"), "slug": slugify(_("SMS 1")), "fields": []}
                groups.append(group)
                current_group = group

        for f in data["fields"]:
            if f["component"] == "group":
                group = {"name": f["label"], "slug": slugify(f["label"]), "fields": []}
                current_group = group
                groups.append(group)
                continue

            field = {"tag": f["label"], "description": f["description"]}
